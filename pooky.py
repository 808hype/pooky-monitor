...
def parse_html(html):
    ...
    for script in tohru_scripts:
        if "supremetohru" in script:
            ...
            tohru = tohru.replace("\"", "")
            tohru = tohru.replace(";", "")
    return pooky, tohru, region


def send_webhook(discordWebhook, slackWebhook, pooky, tohru, region):
    headers = {"Content-Type": "application/json"}
    if tohru is None:
        tohru = "Unknown"
    if not discordWebhook == "":
        payload = {"embed": {"title": "[New Pooky Detected!](" + pooky + ")", "color": 3553599, "timestamp": str(datetime.datetime.now().isoformat()), "fields": [{"name": "Region", "value": region}, {"name": "Tohru", "value": tohru}]}}
    ...
    if not data[4]:
        while True:
            ...
            if parsed is None:
                ...
                hooked = send_webhook(data[0], data[1], parsed[0], parsed[1], parsed[2])
                break
            time.sleep(data[3])
    else:
        proxylist = data[5]
